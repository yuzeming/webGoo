...
setInterval(function(v) {
      ...
      for (j in V) {
        ...
        if (+j < i) {
          ...
          v = (+j && d < 2 * A ? (2 * A - d) / d / 2 : 0) // Collision between o and p
              - (o[4][j] ? C * (d - sprlen) : 0);              // Spring force
  ...
  if (T) {
    ...
    // } else 
    {
      ...
      neighbors.sort(function(a,b) {return (Math.pow(V[a][0] - V[0][0], 2) + Math.pow(V[a][1] - V[0][1], 2) - Math.pow(V[b][0] - V[0][0], 2) - Math.pow(V[b][1] - V[0][1], 2) )}); 
      
      if (neighbors[0]) {
        if (neighbors [1]) {
          q[neighbors[1]] = 1; 
          dis[neighbors[1]] = Math.sqrt(Math.pow(V[0][0] - V[neighbors[1]][0], 2) + Math.pow(V[0][1] - V[neighbors[1]][1], 2)); 
        }
        q[neighbors[0]] = 1; 
        dis[neighbors[0]] = Math.sqrt(Math.pow(V[0][0] - V[neighbors[0]][0], 2) + Math.pow(V[0][1] - V[neighbors[0]][1], 2)); 
